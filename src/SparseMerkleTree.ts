 ...
 */
export class SparseMerkleTree extends Base {
  ...
  static marshalLeaves (leaves: any[]):string {
    return JSON.stringify(leaves.map(leaf => SparseMerkleTree.bufferToHex(leaf)), null, 2)
  }
  ...
  static unmarshalLeaves (jsonStr: string | object):Buffer[] {
    ...
    return parsed.map(SparseMerkleTree.bufferify)
  }
  ...
  */
  ...
  static marshalProof (proof: any[]):string {
    const json = proof.map(item => {
      ...
      if (Buffer.isBuffer(item)) {
        return SparseMerkleTree.bufferToHex(item)
      }

      return {
        position: item.position,
        data: SparseMerkleTree.bufferToHex(item.data)
      }
    })
  ...
  static unmarshalProof (jsonStr: string | object):any[] {
    ...
    return parsed.map(item => {
      if (typeof item === 'string') {
        return SparseMerkleTree.bufferify(item)
      } else if (item instanceof Object) {
        return {
          position: item.position,
          data: SparseMerkleTree.bufferify(item.data)
        }
      } else {
  ...
  static marshalTree (tree: SparseMerkleTree):string {
    const root = tree.getHexRoot()
    const leaves = tree.leaves.map(leaf => SparseMerkleTree.bufferToHex(leaf))
    const layers = tree.getHexLayers()
  ...
  static unmarshalTree (jsonStr: string | object, hashFn = SHA256, options: Options = {}):SparseMerkleTree {
    let parsed :any = null
    if (typeof jsonStr === 'string') {
      parsed = JSON.parse(jsonStr)
    } else if (jsonStr instanceof Object) {
    ...
    return new SparseMerkleTree(parsed.leaves, hashFn, options)
  }
  ...
  static verify (proof: any[], targetNode: Buffer | string, root: Buffer | string, hashFn = SHA256, options: Options = {}):boolean {
    const tree = new SparseMerkleTree([], hashFn, options)
